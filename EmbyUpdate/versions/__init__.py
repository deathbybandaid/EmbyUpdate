...
class Versions():
    ...
    def __init__(self, settings, logger, web):
        ...
        self.github_org_list_url = "https://api.github.com/orgs/EmbyUpdate/repos?type=all"
        self.github_EmbyUpdate_core_info_url = "https://raw.githubusercontent.com/EmbyUpdate/EmbyUpdate/main/version.json"

        self.dict = {}
        self.official_plugins = self.db.get_EmbyUpdate_value("versions", "dict") or {}
    ...
    def sched_init(self, EmbyUpdate):
        """
        The Scheduled update method.
        """

        self.api = EmbyUpdate.api
    ...
    def get_online_versions(self):
        ...
        online_plugin_names = [x["name"] for x in github_org_json if x["name"].startswith("EmbyUpdate_plugin_")]
        for plugin_name in online_plugin_names:
            ...
            for branch in ["main", "master", "dev"]:
                ...
                if not plugin_version_check_success:
                    ...
                    plugin_json_url = "https://raw.githubusercontent.com/EmbyUpdate/%s/%s/plugin.json" % (plugin_name, branch)
                    try:
        ...
        core_json = self.web.session.get(self.github_EmbyUpdate_core_info_url).json()
        for key in list(core_json.keys()):
            self.official_plugins[key] = {"name": key, "version": core_json[key], "type": "core"}

        self.db.set_EmbyUpdate_value("versions", "dict", official_plugins)
